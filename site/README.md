...
| リンク | `#0077c6`・**太字＋下線** | **同値** |
| レイアウト | 本文 716px ＋ サイドバー 304px | 同じ2カラム（900px 以下で1カラム）。**サイドバーを出さないページ（トップ・404）は1カラムで本文 860px** |
...
- **タグラインはヘッダーから外した**（参考サイトに無いため）。`<title>` とトップの h1 に出している
- **ヘッダーのナビはカテゴリだけ**にした（2026-08-31）。運営者情報・プライバシーポリシー・
  サイトマップは「ユーティリティリンク」で、置き場所はフッター（NN/g *Web Page Footers 101*）。
  **実測：375px で6項目のナビは scrollWidth 706px／表示幅 338px、368px ぶんが画面外**に出ていた
  （しかもスクロールバーは非表示にしてある）。カテゴリ3件にしたら溢れは 15px まで減った
...
| 機能 | 実装 |
|---|---|
| グローバルナビ | **カテゴリのみ**。`content/site.json` の `categories` から生成（`nav` はフッター専用） |
| パンくず | ホーム › カテゴリ › 記事 |
| アイキャッチ | front matter の `eyecatch`。og:image と JSON-LD の image も兼ねる |
| 目次 | h2/h3 から自動生成。本文冒頭とサイドバーの2箇所に出力するが、**同時に見えるのは片方だけ**（901px以上＝サイドバー版、900px以下＝本文中版）。以前は両方出ていて同じ目次が2回並んでいた |
...
| 記事カード | サムネイル（240px）＋カテゴリラベル＋説明＋更新日 |
| サイドバー | 目次／このサイトについて／新着記事／カテゴリー。**トップと404では出さない**（中身が本文の複製にしかならないため） |
| トップページ | h1 はタグライン（サイト名を繰り返さない）＋カテゴリカード＋新着記事 |
| フッターナビ | **カテゴリ＋固定ページの全部**（ヘッダーはカテゴリだけ） |
...
### ★★ 図版は PowerPoint で作る（2026-09-01 に全31枚の移行が完了）
...
**★ 31枚すべてが PowerPoint 版。今後の図版もこの方法で作る。**

⚠️ **`tools/svg-to-png.py` は移行済みの図を焼こうとすると止まる。**
PowerPoint で作った PNG を古い SVG の見た目で上書きしてしまうため。
SVG は消していない——**数値の自動照合の相手として残している**
（新しく起こした図は照合相手がいないので、build がその旨を出す）。

**ビルドが落とす検査（3つ）**
1. 図に書いた数値が元の SVG に無い → 落ちる（転記ミスと、根拠のない数字を捕まえる）
2. 見出しが枠に収まらない → 落ちる（LibreOffice が黙って折り返して下の行に重なる）
3. 文字同士が重なる／キャンバスからはみ出す／脚注の罫線に乗る → 落ちる

### 記事が読み込むのは PNG。SVG は素材（2026-08-31 に変更）

```
public/img/*.svg   ← 正（手書きのマスター）。**配信しない**
public/img/*.png   ← tools/svg-to-png.py が焼く。記事が参照するのはこちら
public/img/og/*.png ← SNSカード用。tools/make-og-cards.py が別に作る（従来どおり）
```

**なぜ PNG にしたか**：SVG の文字は `x`/`y` を直接指定して置いてある。
**フォントの無い端末では文字幅が変わり、隣とぶつかったり枠外に出たりする。**
2026-08-31 に `getBBox()` で全31枚を検査したところ、**2枚（`coolerbox-yoryo-eyecatch` と
`coolerbox-yoryo-danmen`）で実際に重なりとはみ出しが起きていた。**
...
- **Chrome ヘッドレスで描く。** 日本語のフォント指定を正しく解釈できるレンダラが要る
  （cairosvg / rsvg は日本語が弱い）。**表示確認に使ったのと同じエンジンで焼く**
- **2倍（`SCALE`）で焼いて等倍で表示する。** 高解像度画面でぼけない
- **256色に減色（`COLORS`）。** 平面塗りの図なので劣化しない。
  実測：154KB → 59KB（−62%）／最大差 34/255・差が8を超える画素は 0.10%（文字の縁だけ）
- 全31枚で 4.5MB → **1.7MB**

⚠️ **図を直すときは SVG を直してから焼き直す。PNG を直接いじらない。**
⚠️ **焼く前に必ず下の `getBBox()` 検査を通す。崩れたまま焼くと固定される。**
⚠️ **`dist/img/*.svg` は build.mjs が削除している。** `.assetsignore` ではできない
（あれは `assets.directory`＝`dist` の直下に無いと読まれず、build.mjs は `public/` しかコピーしない）。
...
| ファイル | 用途 |
|---|---|
| `coolerbox-500ml-eyecatch.svg` | 記事01のアイキャッチ（1200×630）。内寸22×39cmに丸型ボトルが5列3行で並ぶ図 |
| `coolerbox-floorplan.svg` | 本文の図版。**床面積が同じ858cm²でも15本と12本に分かれる**ことを示す図 |
| `favicon.svg` | 寸法線のアイコン。**これは img/ の外なので SVG のまま配信される** |
...
| 表5つ | **全て `.table-wrap` の中で横スクロール**（本文は横に伸びない）。**先頭列は `position: sticky` で固定**し、左右端にスクロールできる合図の影を出す（NN/g *Mobile Tables*：行の見出しを固定する／スクロールできることを見た目で示す）。検証：本文716pxの早見表を `scrollLeft=400`（最大759）にしても先頭セルは左端に残る |
